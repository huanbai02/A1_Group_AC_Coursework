...
\Ensure Predicted labels $\hat{y}_{test}$ and evaluation metrics
\State Fit TF-IDF unigram/bigram vectorizer on $X_{train}$ only.
\State Transform $X_{train}$ into training vectors $V_{train}$.
\State Train \texttt{MultinomialNB} on $(V_{train}, y_{train})$.
\State Transform $X_{test}$ using the fitted vectorizer to obtain $V_{test}$.
\State Predict test labels $\hat{y}_{test}$ with the trained classifier.
...
\State Fit TF-IDF unigram/bigram vectorizer on $X_{train}$ only.
\State Transform $X_{train}$ into training vectors $V_{train}$.
\State Train \texttt{LinearSVC} on $(V_{train}, y_{train})$.
\State Transform $X_{test}$ using the fitted vectorizer to obtain $V_{test}$.
\State Predict test labels $\hat{y}_{test}$ with the trained SVM.
\State Compute precision, recall, F1-score, and accuracy.
\end{algorithmic}
\end{algorithm}
...
\Require Training texts $X_{train}$, training labels $y_{train}$, test texts $X_{test}$
\Ensure Predicted labels $\hat{y}_{test}$ and evaluation metrics
\State Tokenize training texts into token lists $T_{train}$.
\State Train Word2Vec only on $T_{train}$.
\For{each document $d$ in training and test texts}
    ...
    \EndIf
\EndFor
\State Train Logistic Regression on training document vectors and $y_{train}$.
\State Predict test labels $\hat{y}_{test}$ from test document vectors.
\State Compute precision, recall, F1-score, and accuracy.
\end{algorithmic}
\end{algorithm}
